...
def tieu_bien(index,he,nguon_goc): # Tiêu biến mang index tương ứng trong hệ
    ...
    # Tạo hàm tìm index của gốc dương và gốc âm
    index_g = lambda value, he: [i for i in range(len(he)) if he[i][index] == value] 

    # Tạo một hàm tiêu biến với đầu vào là hệ bất phương trình và index tiêu biến, đầu ra là hệ sau tiêu biến

    he_goc = [he[i] for i in range(len(he)) if nguon_goc[i] == 0] # bpt được chọn làm gốc phải có nguồn từ bpt

    max_tem = max([i[index] for i in he_goc]) # Tìm hệ số max gốc dương
    #print(max_tem)

    min_tem = min([i[index] for i in he_goc]) # Tìm hệ số min gốc dương
    #print(min_tem)

    index_d = index_g(max_tem,he)[0] # index của gốc dương 
    #print(index_d)

    index_a = index_g(min_tem,he)[0] # index của gốc âm
    #print(index_a) 
    ...
    for k in range(len(he)):
        if k != index_d:
            if he[k][index] > 0:
                he_moi.append(cong_2list(he[k]*abs(min_tem/he[k][index]),he[index_a]))
            elif he[k][index] < 0:
                he_moi.append(cong_2list(he[k]*abs(max_tem/he[k][index]),he[index_d]))
            else:
                he_moi.append(he[k])
...
def Tim_key_ct(lib_htb,lib_nghiem,order,choice_key): # Ham giai ra tat ca bien khi thay key cua bien cuc tri
	...
	for t in range(2,len(order)):
		...
		for m in range(len(key2)):
			...
			pho1 = phogia(order[-t-1],he_bpt1) #Tim pho
			#print(pho1)

			if len(pho1) != 0:
				for i in range(len(pho1)):
					lib_nghiem["/" + str(i) + key2[m]] = pho1[i]
...
def Giai_he_bpt(he_bpt,biens,cuctri): # Tao he truy xuat giai he bpt
	...
	he = mak_hebpt_matrix(he_bpt,biens) # Tao ma tran tu he bpt
	#print(he)
...
he_bpt = ["2*m+5*g+7*h+10*c<=100","2*m+5*g+10*h+15*c<=100", "4*m+8*g+11*h+19*c-G==0",
			"m>=1","g>=1","h>=1","c>=1"]
...
'''
biens = ["a","b"] # Khai báo tên biến
cuctri = [0,0]	# Khai báo biến có phải cực trị (0: không phải, 1: cực đại, -1: cực tiểu)
he_bpt = ["2*a+b<=6", "a+b>=0", "a-b>=1"]
